...
void draw(){
  ...
  for(i=0;i<choices;i++) result[i] = -1;
  for(m=0;m<choices*choices;m++){
    resultIndex = 0;
    winner = m;
    while(winner>=choices) winner-=choices;
    for(z=0;z<choices*choices;z++){
      for(i=0;i<choices;i++){
        for(j=0;j<choices;j++){
          if(strongest[winner][j] < strongest[j][winner]){
            temp = 1;
            for(k=0;k<choices;k++) if (result[k] == j) temp = 0;
            if(temp == 1) winner = j;
          }
        }
      }
  ...
  /*for(k=0;k<choices;k++){
   ...
   for(z=0;z<choices;z++){
    //choices-n-1;
    if(winner >= choices) winner-=choices;
    for(n=0;n<choices;n++){
      for(i=0;i<choices;i++){
        for(j=0;j<choices;j++){
          if(winner!=j){
            if(strongest[winner][j] < strongest[j][winner]){
              temp=0;
              for(m=0;m<choices;m++) if(result[m] == j) temp = 1;
              if(temp == 0){
                winner = j;
                j=choices;
              }
      ...
      temp = 0;
      for(m=0;m<choices;m++) if(result[m] == winner) temp = 1;  
      if(temp == 0){
        result[resultIndex] = winner;
        resultIndex++;
      }
    */
    for(p=0;p<choices;p++) print( result[p] +" ");
    println("");
  //}
        ...
        infiniteLoop[4] = 1;
        count = 0;
        recursive(4, 1, 4, 9999, infiniteLoop);
        int maxStrength = 0;
        for(k=0;k<strengthsIndex;k++) {
          if(strengths[k] > maxStrength) maxStrength = strengths[k]; 
          println("4 to 1 = "+strengths[k]);
